...
function initCloudant() {
    var cloudantURL = appEnv.services.cloudantNoSQLDB[0].credentials.url || appEnv.getServiceCreds("Cloudant-Test-Mabe").url;
...
function initConversation() {
    ...
    if (!conversationWorkspace) {
      const workspaceName = 'IdiomaMabeConversation';
...
function findAccount(owner){
    Cloudant.db.create("info-account", function(err, body) {
        if (err) {
            console.log("Database already exists: ", "info-account");
        } else {
            console.log("New database created: ", "info-account");
        }
    });
    Cloudant.db.use('info-account').find({selector:{
                    $or:[
                        {"email":owner},
                        {"userred":owner}
                        ]
       ...
       nombre=result.docs[0].fname
       apellido=result.docs[0].lname
       rfc=result.docs[0].rfc
       email=result.docs[0].email
       dob=result.docs[0].dob
       country= result.docs[0].pais
       region=result.docs[0].lacation
       console.log('userRed '+userRed+' nombre '+nombre+' apellido '+apellido+' rfc '+rfc+' email '+email+' dob '+dob+' country '+country+' region '+region);
        ...
        buildContextObject(req, function(err, params) {
            ...
            if (params.message) {
                ...
                conversation.message(params, function(err, data) {
                    ...
                    console.log("-----------------------")  
                    console.log('userRed-message: '+userRed+' nombre-message: '+nombre+' apellido-message: '+apellido+' rfc-message: '+rfc+' email-message: '+email+' dob-message: '+dob+' country-message: '+country+' region-message: '+region);
                    console.log("-----------------------")  
                    
                    data.context.userRed=userRed;
                    data.context.nombre=nombre;
                    data.context.apellido=apellido;
                    data.context.rfc=rfc;
                   data.context. email=email;
                    data.context.dob=dob;
                   data.context. country=country;
                   data.context.region=region;
                   console.log("-----------------------")  
                   console.log('userRed-context: '+data.context.userRed+' nombre-context: '+data.context.nombre+' apellido-context: '+data.context.apellido+' rfc-context: '+data.context.rfc+' email-context: '+ data.context. email+' dob-context: '+ data.context.dob+' country-context: '+data.context. country+' region-context: '+data.context.region);
                    console.log("-----------------------")  

                   if(data.context.indicador==1){
                        mailSender.setEmail(arrayMabInv,arraySurvey,userRed,nombre,apellido,rfc,email,dob,country,region,data.context.indicador);
                        data.context.ssFlag=false;
                   }

                   if(data.context.indicador==4){
                        mailSender.setEmail(arrayMabInv,arraySurvey,userRed,nombre,apellido,rfc,email,dob,country,data.input.text,data.context.indicador);
                        
                   }
                    console.log("Got response from Ana: ", JSON.stringify(data));
                    console.log('Lo que tiene params.input.text '+params.input.text);
                    if(data.context.isSurvey==true){

                            arraySurvey.push(params.input.text);
                    }
                     console.log("-----------------------") 
                     console.log("tamaño del arraySurvey "+arraySurvey.length+"\n contenido arraySurvey "+arraySurvey) 
                      console.log("-----------------------")  
                    if(arraySurvey.length===6 && data.context.indicador==2){
                        mailSender.setEmail(arrayMabInv,arraySurvey,userRed,nombre,apellido,rfc,email,dob,country,region,data.context.indicador);
                         while (arraySurvey.length) {
                             arraySurvey.pop();
                         }
                         data.context.isSurvey=false;
                    }
                    if(data.context.usrMabe===true){
                        arrayMabInv.push(params.input.text);
                    }
                    if(arrayMabInv.length===4 && data.context.indicador==0){
                        mailSender.setEmail(arrayMabInv,arraySurvey,userRed,nombre,apellido,rfc,email,dob,country,region,data.context.indicador);
                         while (arrayMabInv.length) {
                             arrayMabInv.pop();
                         }
                         data.context.usrMabe=false;
                    }
                    console.log("-----------------------") 
                     console.log("tamaño del arrayMabInv "+arrayMabInv.length+"\n contenido arrayMabInv "+arrayMabInv) 
                      console.log("-----------------------")  
                    updateContextObject(data, userPolicy,owner,userRed,nombre,apellido,rfc,email,dob,country,region, function(err, res) {
 ...
 */
function updateContextObject(response, userPolicy,owner,userRed,nombre,apellido,rfc,email,dob,country,region, callback) {
    ...
    response.context.username=owner
    response.context.userRed=userRed
    response.context.nombre=nombre
    response.context.apellido=apellido
    response.context.rfc=rfc
    response.context.email=email
    response.context.dob=dob
    response.context.country=country
    response.context.region=region
    if(response.input.text=='si'||response.input.text=='no'){
    response.context.username="";
    response.context.userRed="";
    response.context.nombre="";
    response.context.apellido="";
    response.context.rfc="";
    response.context.email="";
    response.context.dob="";
    response.context.country="";
    response.context.region="";
    userRed="";
    nombre="";
    apellido="";
    rfc="";
